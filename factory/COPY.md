...
5. Copy `factory/project.json`. Pick writer harness, isolate, `access`, `commitCredit`, `promptCache`. The control plane launches the writer (D-04, D-33). A `hands.mjs` check is not a copy gate. Claude Code absent is the default. Token-only is `pc-token` or `cloud-grok`. A child never writes VirBk/Grok.
